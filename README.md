...
In this level we now have two dialects: `accel_host` and `accel_high`. One is for the host-side of the code that interacts with the accelerator, and the second is for representing the high-level accelerator operations. The `accel_host.launch` is used as the interface between the host memory and the device memory; hence you can see the matrices enter the operations as memrefs, but then get converted to our custom device tensors within the scope of the operation. This strictly enforces that the memory inside the scope is different from the memory outside and observes different constraints.
...
Now, what we have is a dialect entry point which can be targeted by any frontend that targets the Linalg dialect; but more importantly, we have stepping stone dialects that can be trivially converted from one to the other. This ensures that any code that enters the Linalg dialect will exit as a working heterogenous program. This is how we design heterogenous compilers in MLIR.
